...
graph TD
    ...
    B --> C[Owner: Đăng ký ngựa & Mời Jockey]
    C --> D[Vet: Khám sức khỏe ban đầu - Initial Check]
    
    D -->|Khám đạt: Pass & Doping Negative| E[Admin: Duyệt đơn đăng ký]
    D -->|Khám hỏng: Fail hoặc Doping Positive| F[Hệ thống: Loại ngựa khỏi giải - Disqualified]
    
    E --> G[Admin: Đóng cổng đăng ký]
    G --> H{Kiểm tra số lượng ngựa đã duyệt?}
    
    H -->|Số ngựa ≤ 12| I[Tự động tạo 1 trận Chung kết - Final Race]
    H -->|Số ngựa > 12| J[Tự động chia đều ngựa vào các trận Vòng loại - Pre Round]
    
    I --> K[Admin: Phân công Trọng tài vào trận]
    J --> K
    
    K --> L[Spectator: Đặt cược hoặc Dự đoán]
    L --> M[Trận đấu diễn ra]
    M --> N[Referee: Nhập kết quả & Nộp báo cáo]
    
    N --> O{Kiểm tra vòng đấu vừa chạy?}
    O -->|Nếu vừa chạy Vòng loại| P[Hệ thống: Lọc Top 12 ngựa tốt nhất]
    P --> I
    
    O -->|Nếu vừa chạy Chung kết| Q[Admin: Trả thưởng cược & Giải thưởng giải đấu]
    Q --> R[Hệ thống: Kết thúc giải đấu - Status: Completed]
```
...
2. **Hành động**:
   * Chủ ngựa thực hiện đăng ký ngựa (Xem tại [01-subflow-horse-registration.md](file:///d:/SU26_semeter5/SWP/HorseRacingManagementSystem/.antigravity/scripts/01-subflow-horse-registration.md)).
   * Chủ ngựa ký hợp đồng mời Jockey (Xem tại [02-subflow-jockey-invitation.md](file:///d:/SU26_semeter5/SWP/HorseRacingManagementSystem/.antigravity/scripts/02-subflow-jockey-invitation.md)).
3. **Trực quan hóa**:
   * Một dòng đăng ký mới (`Registration`) được tạo ở trạng thái `Pending`.
   * Một hợp đồng kỵ sĩ (`JockeyContract`) ở trạng thái `Accepted`.
...
2. **Hành động**:
   ...
   * Điền kết quả khám: Cân nặng, Nhịp tim, Nhiệt độ và Doping.
3. **Điều kiện quyết định duyệt**:
   * **Nếu khám ĐẠT (`Pass` & `Negative` Doping)**: Admin có thể ấn nút duyệt (`Approve`) đơn đăng ký của ngựa.
   * **Nếu khám KHÔNG ĐẠT (`Fail` hoặc `Positive` Doping)**: Đơn đăng ký tự động bị từ chối/loại bỏ (`Disqualified`).
   * **Ràng buộc cứng trong Code**: Admin không thể phê duyệt một đơn đăng ký nếu hợp đồng Jockey tương ứng chưa đạt trạng thái `Accepted`.
